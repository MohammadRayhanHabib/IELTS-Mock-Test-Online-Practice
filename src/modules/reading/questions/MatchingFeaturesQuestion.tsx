...
const MatchingFeaturesQuestion: React.FC<ReadingQuestionComponentProps> = ({ question, answer, onChange, firstQuestionNumber, visualVariant, textClassName, flaggedQuestions, onToggleFlag }) => {
  ...
  return (
    <div className="space-y-6">
      ...
      <MatchingInformationGrid questionId={question._id} statements={question.options ?? []} columnLabels={choices.map((choice) => choice.letter)} answer={answerAsArray(answer)} onChange={onChange} firstQuestionNumber={firstQuestionNumber} statementHeader="Statements" selectionLabel="feature" visualVariant="features" flaggedQuestions={flaggedQuestions} onToggleFlag={onToggleFlag} />
    </div>
  );
};
