...
const MatchingFeaturesQuestion: React.FC<ReadingQuestionComponentProps> = ({ question, answer, onChange, firstQuestionNumber, visualVariant, textClassName }) => {
  const choices = parseStatementMatchChoices(question.wordBank);
  if (visualVariant !== "client-preview") {
    return <StatementMatchingPanel questionId={question._id} statements={question.options ?? []} wordBank={question.wordBank ?? []} answer={answerAsArray(answer)} onChange={onChange} firstQuestionNumber={firstQuestionNumber} />;
  }
  return (
    <div className="space-y-6">
      ...
      <MatchingInformationGrid questionId={question._id} statements={question.options ?? []} columnLabels={choices.map((choice) => choice.letter)} answer={answerAsArray(answer)} onChange={onChange} firstQuestionNumber={firstQuestionNumber} statementHeader="Statements" selectionLabel="feature" visualVariant="features" />
    </div>
  );
};
