...
const McqSingleQuestion: React.FC<ReadingQuestionComponentProps> = ({ question, answer, onChange, firstQuestionNumber, textClassName, visualVariant, flaggedQuestions, onToggleFlag }) => {
  const grouped = (question.options ?? []).some((row) => row.includes("|||"));
  return grouped ? (
    <QuestionChoiceGroup questionId={question._id} rows={question.options ?? []} answer={answerAsArray(answer, question.options?.length)} onChange={onChange} firstQuestionNumber={firstQuestionNumber} textClassName={textClassName} visualVariant={visualVariant === "client-preview" ? "single-choice-reference" : "default"} flaggedQuestions={flaggedQuestions} onToggleFlag={onToggleFlag} />
  ) : (
