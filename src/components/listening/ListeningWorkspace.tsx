...
interface ListeningWorkspaceProps {
  ...
  loadingLabel?: string;
  className?: string;
  variant?: "default" | "official-preview";
}
...
const ListeningWorkspace: React.FC<ListeningWorkspaceProps> = ({
  ...
  const officialPreview = variant === "official-preview";
  ...
  return (
    <div
      className={`flex flex-col h-full w-full ${
        officialPreview ? "max-w-none" : "max-w-7xl mx-auto"
      } ${className}`}
    >
      <audio
        ref={audioRef}
      ...
      <div className={`flex flex-1 min-h-0 ${officialPreview ? "gap-0" : "gap-5"}`}>
        <aside className={officialPreview ? "hidden" : "w-48 shrink-0"}>
        ...
        <div className={`flex-1 bg-white overflow-hidden flex flex-col min-h-0 ${
          ...
          {officialPreview && (
            <nav
              ...
              className="grid shrink-0 grid-cols-2 border-t border-gray-300 bg-white lg:grid-cols-4"
            >
              {test.sections.map((section, sectionIndex) => {
                const sectionRange = sectionListeningRange(test, sectionIndex);
                ...
                return (
                  <button
                    ...
                    className={`border-r border-gray-200 px-4 py-2 text-left last:border-r-0 ${
                      active
                        ? "border-t-4 border-t-rose-700 bg-rose-50/40"
                        : "border-t-4 border-t-transparent hover:bg-gray-50"
                    }`}
                  >
                    ...
                    <span className="block text-xs text-gray-500">
                      Questions {sectionRange.start}–{sectionRange.end}
                    </span>
                  </button>
                );
              })}
              ...
              {stems.map((stem, i) => (
                <li key={i} className="flex items-start gap-3">
                  ...
                  <input
                    ...
                    onDrop={(event) => {
                      event.preventDefault();
                      setCell(i, event.dataTransfer.getData("text/plain"));
                    }}
              ...
              {pool.map((opt, j) => {
                ...
                return (
                  <li
                    ...
                    draggable
                    onDragStart={(event) => {
                      event.dataTransfer.setData("text/plain", L);
                      event.dataTransfer.effectAllowed = "copy";
                    }}
                    className="cursor-grab rounded-lg border border-dashed border-gray-300 bg-white px-3 py-2.5 text-sm leading-snug text-gray-800 active:cursor-grabbing"
                  >
