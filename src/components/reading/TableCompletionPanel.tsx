      ...
      <div className="w-full overflow-x-auto pb-1">
        <table className="w-full min-w-[700px] table-fixed border-collapse text-[16px] leading-[1.35] text-gray-900">
          ...
          <thead>
            ...
            <tr>
              {paddedHeaders.map((header, index) => (
                <th
                  key={index}
                  scope="col"
          ...
          <tbody>
            {paddedBody.map((cells, rowIndex) => (
              <tr key={rowIndex} className={rowIndex === 0 ? "h-[158px]" : "h-[126px]"}>
                {cells.map((_, cellIndex) => {
                  const rawCell = bodyRows[rowIndex]?.[cellIndex] ?? "";
                    ...
                    >
                      <div className="leading-[1.35]">
                        {parts.map((part, partIndex) => {
                          ...
                          const visiblePart = endsWithLineBreak
                            ? part.replace(/\n\s*$/, "")
                            : part;

                          return (
                            <React.Fragment
                              key={`${rowIndex}-${cellIndex}-${partIndex}`}
                            >
                              {visiblePart ? (
                              ...
                              {partIndex < parts.length - 1 ? (
                                ...
                                <span className="relative inline-flex h-[30px] w-full min-w-0 max-w-full overflow-hidden border border-gray-500 bg-white">
                                  ...
                                  <input
                                    ...
                                    className="ielts-numbered-answer-input relative z-[1] h-full min-w-0 flex-1 bg-transparent px-2 text-center font-semibold text-gray-900 focus:outline-none"
                                    placeholder=""
                                    autoComplete="off"
                                  />
                                </span>
