...
export interface MatchingInformationGridProps {
  ...
  /** Accessible label for the selectable column value. */
  selectionLabel?: string;
  /** When true, radios are disabled (e.g. admin preview). */
  readOnly?: boolean;
  visualVariant?: "default" | "reference" | "features";
    ...
    <div className={referenceVariant ? "relative pr-10" : ""}>
      ...
      <table
        className={`w-full border-collapse text-gray-900 ${
          featuresVariant
            ? "min-w-[19rem] text-base"
            : referenceVariant
              ? "min-w-[700px] text-base"
              : "min-w-[320px] text-sm"
        }`}
      >
        <thead>
          ...
          >
            <th
              ...
              className={
                featuresVariant
                  ? "sticky left-0 z-[1] min-w-[9rem] border-r border-gray-300 bg-white px-3 py-3 text-center font-bold text-gray-900"
                  : referenceVariant
            ...
            return (
              ...
              >
                <td
                  className={
                    featuresVariant
                      ? "sticky left-0 z-[1] min-w-[9rem] max-w-[24rem] border-r border-gray-300 bg-white px-3 py-4 align-middle leading-snug"
                      : referenceVariant
                  ...
                  >
                    ...
                    >
                      <input
                        ...
                        aria-label={`Question ${qn}: ${selectionLabel} ${col}`}
                      />
                    </label>
                  </td>
                ))}
      ...
      {referenceVariant && showBookmark && (
        <button
          ...
          className={`absolute right-0 top-[4.75rem] flex h-8 w-8 items-center justify-center bg-transparent transition-colors ${
            bookmarked
